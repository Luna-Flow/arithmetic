...
`Float` 与 `Double` 把 contextual 边界适配到现有原生和 checked 操作上。除法与
平方根复用 checked 验证；整数嵌入检测原生 `Float` 转换损失，相邻值操作按 IEEE
位表示移动，固定格式中的邻接选择作为精确 capability 操作返回空 diagnostics。

这些适配器只保证接口可用，并不是软件 decimal 引擎。它们不会假装支持任意精度、
定向舍入、指数钳制或完整 IEEE 状态标志生成。
它们有意不实现 `ConstantsContextual` 或 `HyperbolicContextual`。采用证明式数值计算的包
可以实现这些能力，并通过 `ArithmeticError` 保留认证失败细节。

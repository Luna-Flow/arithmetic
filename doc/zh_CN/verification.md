...
contextual tests 覆盖精确与发生舍入的 `Int` 嵌入、带符号零方向、NaN 传播、静止
infinity、有限值到 infinity 的步进，以及固定 IEEE `Float`/`Double` 格式中最小
subnormal 到零的步进。测试包内的实现还验证 contextual 双曲结果和常量认证失败传播。

测试不声称 `Float` 或 `Double` 执行 certified arithmetic；它们只验证共享 capability
vocabulary、固定格式的相邻值语义及其普通 error-model 行为。
